package com.example.aula08

import android.os.Bundle
import androidx.fragment.app.Fragment
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import androidx.appcompat.app.AlertDialog
import com.example.aula08.databinding.ActivityMainBinding
import com.example.aula08.databinding.FragmentTeste01Binding

class Teste01Fragment : Fragment() {
    //Binding Automatico no Fragmento
    lateinit var binding: FragmentTeste01Binding
    override fun onCreateView(inflater: LayoutInflater, container:ViewGroup?, savedInstanceState: Bundle?): View? {
        //Binding Automatico no Fragmento
        binding = FragmentTeste01Binding.inflate(inflater)

        binding.buttonMsg.setOnClickListener {

            //contexto em fragmento usar requireContext()
            AlertDialog.Builder(requireContext())
                .setMessage("Ola, Mundo")
                .setPositiveButton("OK",null)
                .create()
                .show()
        }
        return binding.root
    }

}